...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="If set, force every reset to use this seed (eval the policy on the "
             "track it was curriculum-trained on, or any specific track).",
    )
    p.add_argument(
        "--waypoint-bonus",
        type=float,
        default=PitwallRacingEnv.WAYPOINT_BONUS_DEFAULT,
        help="Waypoint bonus during eval. Set to 0 for apples-to-apples reward "
             "comparison with pre-waypoint runs.",
    )
    p.add_argument(
